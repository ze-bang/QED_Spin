...
We build a 2-site Heisenberg chain by hand, run quantum_ed.full_diagonalization,
...
def _build_heisenberg_2site() -> "quantum_ed.Operator":
    """H = J ( S+0 S-1 + S-0 S+1 + Sz0 Sz1 ), J=1."""
    op = quantum_ed.Operator(num_sites=2, spin=0.5)
    one = complex(1.0, 0.0)
    half = complex(0.5, 0.0)

    op.add_two_body(quantum_ed.OP_SPLUS,  0, quantum_ed.OP_SMINUS, 1, half)
    op.add_two_body(quantum_ed.OP_SMINUS, 0, quantum_ed.OP_SPLUS,  1, half)
    op.add_two_body(quantum_ed.OP_SZ,     0, quantum_ed.OP_SZ,     1, one)
    return op
...
def test_2site_heisenberg_spectrum_matches_analytic():
    ...
    eigvals = quantum_ed.full_diagonalization(op)
...
def test_lanczos_ground_state_matches_full_diag():
    op = _build_heisenberg_2site()
    full = np.sort(np.asarray(quantum_ed.full_diagonalization(op)))
    e0_lanczos = quantum_ed.lanczos(op, max_iter=50, exct=1, tolerance=1e-12)

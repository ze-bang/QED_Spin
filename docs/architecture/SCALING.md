...
> The authoritative list of environment variables is `include/ed/config/env_registry.h`;
> `qed.debug_env()` prints every one with its live value, default and meaning. The tables
> below explain the ones that matter for scaling. A misspelt `ED_*` name warns at import.

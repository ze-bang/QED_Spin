...
| 11c-2a | **Eager builders deleted outright** (Jul 2026): the "test oracle" status of 11c-1 lasted one commit — what those suites pin (dims, tiling, canonical ordering, Bethe GS vs dense) is lane-independent physics, and the true bit-identity oracles are the legacy enumerators in `test_compiled_group`/`test_orbit_table`. The 12 eager call sites across 8 unit suites now build through the lazy rep-first builders (materialising via `ensureHostCsr()` where they inspect orbit data — the ordering/sector_id contracts are now pinned ON the production lane); `build_full/fixed_sz_sector_operators`, `make_sector_operator_adopt`, and `SectorBasis::adopt` deleted (~11 kB); the two adopt-bridge TEST_CASEs died with their subject. `SectorBasis` now has exactly two construction routes: `configureRepLazy` (production) and `build` (the lazy `csr_provider`'s on-demand materialiser). Deferred to 11c-2b: retiring the orbit matvec-backend escapes (`ED_SYM_REP`/`ED_GPU_SYMMETRY_REP`) — after adopt removal they are the ONLY route into the CPU orbit backend / GPU orbit mirror. Rider fix (found chasing a one-off suite flake with 100%-wrong spectra): the cross-irrep spectral bindings accepted the GS pair with NO convergence check — the orchestrator returns a best-effort pair on non-convergence and the scattered weights + continued fraction silently inherit the garbage; both GS sites now run `require_gs_residual` (one matvec, throw at 1e-8 — the guard `little_group_ground_state` already had) | migrated suites 74/74 (sector/symmetry/thermal_dense/thermal_save/projector_chain/nonabelian_guard); full check_local.sh gate | **done (Jul 2026)** |
| 11c-2b | **ONE matvec representation, host and device** (Jul 2026): the legacy orbit-CSR matvec lane — reachable only through env escapes since 11c-2a — deleted outright. CPU: `make_backend_` is rep-only (`cpu_rep_symmetry_enabled`/`ED_SYM_REP` gone; `make_cpu_symmetry_backend` deleted; unusable rep data now throws as a construction-bug invariant); GPU: `bind_cuda_impl_` is rep-only (`rep_path_enabled`/`ED_GPU_SYMMETRY_REP` gone; `GpuSectorMirror` + `build_mirror` + `launch_symmetry_matvec` + `make_sector_matvec_gpu{,_dev}` + the `ED_GPU_SYMMETRY_MIRROR_V2` tuning helpers deleted, ~27 kB; `make_cuda_symmetry_backend` — the never-consumed P3c device orbit lane — deleted too); the unreachable eager orbit-walk dense assembler in `subspace_operator.h` deleted (rep-CSR densify is the one assembly path); `OrbitMaterialized` removed from `SymMatvecRepr`; `sector_plan`'s flip-project veto and the flip/parity provider messages no longer reference the dead gate. Tests: the orbit-lane pins (`test_symmetry_backend{,_fixedsz}`, `test_cuda_symmetry_matvec_backend`) retired with their subject; `test_rep_lazy_sector_loop` now pins the STRONGER invariant (host CSR stays absent after CPU apply); `test_sector_operator` extracts rep data via `rep_sector_data_from_sector` for its directly-built basis; the integration strategy-parity test compares the two surviving rep sub-modes | rep/sector/symmetry ctest lanes green; full check_local.sh gate | **done (Jul 2026)** |
| 11d-prep | **Operator-class inventory + sediment hygiene** (Jul 2026). Finding: the host-side "one operator" goal is already REAL — the monolithic streaming operators (`StreamingSymmetryOperator` / `FixedSzStreamingSymmetryOperator`, `StreamingSymmetryHandle`, `SectorView`) no longer exist as code, `FixedSzOperator` is an alias of `SubspaceOperator<FixedSzBasisPolicy>`, and the hierarchy is `LinearOperator ← Operator ← SubspaceOperator<Policy>` — but a dozen live headers still DESCRIBED the deleted classes as current (api.h's public docs, make_operator.h's return-type matrix, sector_loop.h, linear_operator.h, subspace.h, sector_basis.h, symmetry_basis_policy.h, cross_sector_orbit_observable.h), exactly the sediment that misleads the next session. All rewritten to describe today's shape (bit-identity provenance mentions kept). Riders: `rep_lazy_sector_path_enabled()` — a straggler still reading the retired `ED_GPU_SYMMETRY_REP` — deleted with its test guard; the dead `extern template CpuMatVecBackend<SymmetryBasisPolicy,…>` (orphaned by 11c-2b's factory deletion) + its explicit instantiation deleted. Remaining 11d items dispositioned: the full `Operator`→template inversion is elegance-churn with no fork elimination (declined); the distributed family (~6.6 kLOC: `DistributedOperator`/`DistributedSymmetryOperator` + distributed lanczos/ftlm/tpq/krylov-schur + GPU twins, consumed ONLY by `ed_distributed_main`) is a user capability decision — production MPI is `MpiBackend` × SectorDistributor | full check_local.sh gate | **done (Jul 2026)** |
| 11d | **Distributed-operator family retired** (Jul 2026, user-approved): `src/distributed/` + `include/ed/distributed/` (~6.6 kLOC: `DistributedOperator`/`DistributedSymmetryOperator` + distributed lanczos/FTLM/TPQ/Krylov-Schur + GPU/NCCL twins + orbit halo/partition primitives), the `ed_distributed_main` cluster launcher, the `OperatorSpec.distributed` axis, and the Python subprocess lane (`qed.mpi`, `device='mpi'/'mpi_gpu'` with its launcher kwargs, `load_mpi_eigenvector(s)`, the Phase-H per-sector Z-weighted aggregation) all deleted; 22 family test files retired. `device='mpi'` now raises with mpirun guidance. Survivor: the NCCL `MultiGpuCommunicator` (relocated to `ed/parallel/multi_gpu.h`, slim library `ed_multi_gpu`) — it is production, consumed by `MpiCudaBackend`/`select_backend`; its two tests (`test_multi_gpu_nccl`, `test_mpi_cuda_backend`) re-registered against the new library, and the production MPI matvec pins (`test_comm_plan`, `test_mpi_matvec_impl`) kept. ONE MPI story remains: `ED` under mpirun — SectorDistributor across sectors × `MpiBackend` in-process. Recoverable from git history if within-sector memory distribution is ever needed | full check_local.sh gate; the mpi-labeled production tests (comm plan, MPI matvec impl, SectorDistributor suites) stay green | **done (Jul 2026)** |

...
and Phase 2 (P2.1–P2.14) have all landed. The codebase now offers exactly
one canonical C++ caller for every ED core routine — `./ED` (with the
built-in `ED dssf <method>` subcommand for spectral / static structure
factors). The only remaining standalone binary is the BFG order-parameter
research add-on (`compute_bfg_order_parameters` / its CUDA twin), which
deliberately stays out of the core CLI because its scope is narrow.
...
**Renaming**: `edlib` → `pyed` (or `qed`, `edpkg`). PyPI-available as of writing for `pyed`. Internal Python API:
...
7. **Phase 2 PR-G**: pybind11 binding `ed.dssf.run(...)` against `ed::dssf::dssf_engine`. `python/qed/dssf.py` provides the high-level Pythonic wrapper.
...
- [x] **P2.1** Refactor `compute_bfg_order_parameters.cpp` into `src/bfg/` library + ~50-line `main`. *Done -- tenth (final) slice landed:* on top of the previous nine slices, this slice re-points the GPU driver (`src/apps/compute_bfg_order_parameters_gpu.cu`) onto the library types. The local `Cluster` struct (with its nested `Cluster::Bowtie` and the bowtie list as a member field), the local `load_cluster` (with its inline triangle / bowtie search and ad-hoc kagome lattice-vector defaults), and the unused-after-removal `<set>` / `<numeric>` includes are gone (~200 LOC). The GPU driver now `using`-aliases `ed::bfg::{Cluster, Bowtie, load_cluster, find_bowties}`, calls `find_bowties(cluster)` once next to each cluster load, and threads the resulting `std::vector<Bowtie>` through `compute_order_parameters_gpu(...)` (new positional argument) and `compute_bowtie_resonances_gpu(...)` (now takes `const std::vector<Bowtie>&` instead of `const Cluster&`). Behavioural consequences: the GPU's k-point grid, bond-orientation map, and bowtie list are now identical to the CPU pipeline by construction (the local copy used to drift -- the local `load_cluster` always defaulted to the kagome lattice vectors and a `dim*dim` k-point grid where `dim = round(sqrt(n_sites/3))`, while the library version reads `n_cells_x/y` and `b1`/`b2` from the lattice-parameters file when available, falling back to the same heuristics only when the file is absent). Net change: GPU driver **1193 -> 1030 LOC** (-163 LOC); CPU driver: unchanged at **183 LOC**. ctest: **102/102 pass** (unchanged); pytest: **81/81 pass** (unchanged). With this slice the audit promise of a single authoritative BFG library driving both binaries (and the Python bindings) is fully satisfied. *(Earlier slice notes preserved below for archaeology.)*

  *Ninth slice (orchestration shell, landed earlier):* on top of the previous eight slices, this slice promoted the orchestration shell itself into `ed_bfg::cli`: `include/ed/bfg/cli.h` + `src/bfg/cli.cpp` ship `print_usage`, `process_all_temperatures`, `scan_jpm_directories(opts, mpi_rank, mpi_size)`, `run_single_file(opts)`, `run_scan(opts, mpi_rank, mpi_size)`, plus the `SingleFileOptions` / `ScanOptions` argv-derived option PODs. With the orchestration moved, the CPU driver (`src/apps/compute_bfg_order_parameters.cpp`) became a strict argv shell: it handles MPI_Init/Finalize, parses argv into one of the two option PODs, dispatches to the matching `run_*` helper, prints the wallclock summary, and turns exceptions into a non-zero exit code. Net change: CPU driver **816 -> 183 LOC** (-633 LOC; ~45 of the remaining 183 are the docstring banner). The MPI partitioning that used to live inside `scan_jpm_directories` now flows through the explicit `(mpi_rank, mpi_size)` arguments, which lets `ed_bfg` itself stay MPI-agnostic (no MPI in its PUBLIC link line) while the calling executable still owns whether to compile in MPI support. All numerical, on-disk, and console-progress behaviour was preserved verbatim.

  *Eighth slice (order-parameter physics kernels, landed earlier):* on top of the previous seven slices, this slice promoted the order-parameter physics kernels themselves into `ed_bfg::order_parameters`: `include/ed/bfg/order_parameters.h` + `src/bfg/order_parameters.cpp` ship `compute_nematic_order(_real)`, `compute_vbs_order`, `compute_plaquette_order`, `compute_sq_2d_grid`, and `compute_all_order_parameters`. With every kernel and POD now in the library, the CPU driver collapsed to a thin orchestration shell: `process_all_temperatures` (TPQ-temperature sweep), `scan_jpm_directories` (per-Jpm parallel/MPI distribution), `print_usage`, and `int main()`. The driver also dropped the dead bit / spin-operator helpers (`get_bit`, `set_bit`, `flip_bit`, `apply_sp`, `apply_sm`, `sz_value`) that lingered after earlier kernel extractions, the dead `PI`/`I` constants, and the unreferenced subset of using-declarations / std-headers. Net change: CPU driver **2645 -> 816 LOC** (-1 829 LOC across slices 7-8, of which -1 013 LOC came from this slice alone). All numerical and console-progress behaviour was preserved verbatim (the implementation moved, not the logic).

  *Seventh slice (HDF5 results writers, landed earlier):* on top of the previous six slices (`Cluster` + `load_cluster`, topology + correlations, wavefunction I/O, dimer structure factors, ring observables, spin structure factor), this slice promoted the HDF5 results writers and their scalar-summary record types into `ed_bfg::results_io`: `include/ed/bfg/results_io.h` + `src/bfg/results_io.cpp` ship `save_results`, `save_temperature_scan_results`, `save_scan_results`, plus the canonical struct definitions (`NematicResult`, `VBSResult`, `PlaquetteResult`, `Sq2DGridResult`, `OrderParameterResults`). The on-disk schema is preserved bit-for-bit (HDF5 dataset names, attribute names, the orientation index map `0=(0,0), 1=(0,1), 2=(0,2), 3=(1,1), 4=(1,2), 5=(2,2)`, and the spsm/szsz/heisenberg bond compound types). The CPU driver lost ~816 LOC (2645 -> 1829 LOC). Pybind11 bindings for these record/writer types are intentionally deferred -- the result aggregates are only used internally by the driver, not by user-facing notebooks.

  *Sixth slice (spin structure factor, landed earlier):* on top of the previous slices (`Cluster` + `load_cluster`, topology + correlations, wavefunction I/O, dimer structure factors, ring observables), this slice promotes the site-resolved spin structure factor into `ed_bfg::spin_structure_factor`: `include/ed/bfg/spin_structure_factor.h` + `src/bfg/spin_structure_factor.cpp` ship `compute_spin_structure_factor(smsp_corr, szsz_corr, cluster)` and the `StructureFactorResult` POD (`s_q`, `s_q_smsp`, `s_q_szsz`, `q_max_idx`, `q_max`, `s_q_max`, `m_translation`). The kernel only consumes precomputed two-body correlation tables (`<S^-_i S^+_j>` + `<S^z_i S^z_j>`) and the cluster's k-point list / minimum-image displacement table, so it cleanly separates from any wavefunction handling. The CPU driver loses ~80 LOC (2719 -> 2645 LOC) and now `using`-aliases the new symbols. Catch2 lockdown: `tests/unit/test_bfg_spin_structure_factor.cpp` (3 cases) covers the polarised |up,up> ground state, the Néel |up,down> case (peak at q = π with hand-checked Re/Im), and the empty-k_points edge case. Pybind11 bindings expose `qed.bfg.compute_spin_structure_factor` and `StructureFactorResult` (the latter as a read-only handle); 2 new pytest cases (`python/tests/test_bfg.py`) round-trip the kernel through the kagome 1×1 fixture (zero-correlations -> zero S(q) and uniform SzSz -> peak at Γ with `m_translation = sqrt(<SzSz> N)`).

  *Fifth slice (ring observables, landed earlier):* on top of the previous slices (`Cluster` + `load_cluster`, topology + correlations, wavefunction I/O, structure-factor / Fourier-applied dimer kernels), this slice promotes the bowtie ring-flip + triangle ring-exchange operators into `ed_bfg::ring_observables`: `include/ed/bfg/ring_observables.h` + `src/bfg/ring_observables.cpp` ship `apply_bowtie_fourier(bowties, psi, q)`, `compute_bowtie_resonance(psi, s1..s4)`, and `compute_triangle_chiral(psi, s1, s2, s3)`. The kernel re-uses the memory-efficient flag exported by `structure_factor.h` so the same atomic / thread-local switching policy applies to bowtie Fourier sweeps. **API consolidation:** the file-local `BowtieData {s1,s2,s3,s4,center}` POD that the CPU driver maintained alongside `ed::bfg::Bowtie` is gone -- the kernel now consumes `ed::bfg::Bowtie` directly (only `s1..s4` and `center` are read; `s0` / `orientation` are ignored), so the CPU driver's `BowtieData` rebuild loop drops away. The CPU driver loses ~210 LOC (2929 -> 2719 LOC) and now `using`-aliases every moved symbol. Catch2 lockdown: `tests/unit/test_bfg_ring_observables.cpp` (10 cases) covers the polarised-state vanish for both real-space kernels, the analytic `2 Re(conj(a) b)` matrix elements on `(a|UDU>+b|DUD>)` / `(a|DUDU>+b|UDUD>)` superpositions, the empty-bowtie short-circuit, the q=0 / phase-pickup branches of `apply_bowtie_fourier`, the hermitian-conjugate branch, and a fast-vs-atomic equivalence check that exercises both code paths via the memory-efficient flag. Pybind11 bindings expose the same surface under `qed.bfg` (`apply_bowtie_fourier`, `compute_bowtie_resonance`, `compute_triangle_chiral`); the `Bowtie` class also gained a Python constructor + `def_readwrite` so notebook callers can synthesise bowties without going through `find_bowties`. 8 new pytest cases (`python/tests/test_bfg.py`) round-trip the analytic states from the Catch2 suite through the bindings. ctest: **99/99 pass** (was 89/89); pytest: **79/79 pass** (was 71/71). The remaining ~2 700 LOC is now spin S(q) / nematic / VBS / plaquette / HDF5-results-writer / scan-mode logic; future commits will peel them off in similarly-scoped slices. *(Earlier slice notes preserved below for archaeology.)*

  *Fourth slice (structure factors / Fourier-applied dimer kernels, landed earlier):* on top of the first three slices, this slice promoted the bond-bilinear structure-factor and Fourier-applied dimer kernels into `ed_bfg`: `include/ed/bfg/structure_factor.h` + `src/bfg/structure_factor.cpp` ship `DimerSFResult`, `compute_dimer_sf_direct`, `compute_heisenberg_sf_direct`, `apply_dimer_fourier`, `apply_heisenberg_dimer_fourier`, `compute_dimer_dimer_correlation`, `compute_heisenberg_dimer_dimer_correlation`, plus the `set_memory_efficient_mode` / `memory_efficient_mode_enabled` accessors that wrap the previously file-local global. The CPU driver lost ~660 LOC (3584 -> 2929 LOC). Catch2 lockdown: `tests/unit/test_bfg_structure_factor.cpp` (7 cases). Pybind11 bindings expose the same surface under `qed.bfg`; 7 new pytest cases.

  *Third slice (HDF5 wavefunction loaders, landed earlier):* on top of the previous `Cluster` + topology + correlations extraction, this slice promotes the HDF5 wavefunction loaders into the `ed_bfg` library: `include/ed/bfg/wavefunction_io.h` + `src/bfg/wavefunction_io.cpp` ship `load_wavefunction`, `load_tpq_state`, `load_all_tpq_states`, and the `TPQState` POD. The new loader probes the on-disk compound member names so it round-trips both the legacy `(real, imag)` layout the CPU driver always emitted and the `(r, i)` layout `h5py` produces by default (the GPU driver's earlier ad-hoc fallback used to silently read `(r, i)` files as zeros, which this consolidation fixes); the non-compound path is preserved as a real-only wavefunction reader. Both the CPU driver (`compute_bfg_order_parameters.cpp`) and the GPU driver (`compute_bfg_order_parameters_gpu.cu`) now `using ed::bfg::load_wavefunction;`-alias the library version and the duplicate copies (~280 LOC across the two files) are gone. CMake: `ed_bfg` gained an HDF5 PUBLIC link dep; the GPU driver now links `ed_bfg` directly. Catch2 lockdown: `tests/unit/test_bfg_wavefunction_io.cpp` (7 cases) covers canonical / legacy / real-only on-disk layouts, missing-dataset error paths, lowest-T TPQ snapshot selection, and ascending-T sort ordering. Pybind11 bindings expose `qed.bfg.load_wavefunction`, `load_tpq_state`, `load_all_tpq_states`, and the `TPQState` class with `psi` returned as a NumPy `complex128` array (no per-element conversion); 3 new pytest cases (`python/tests/test_bfg.py`) round-trip wavefunctions through real `h5py` files. ctest: **82/82 pass**; pytest: **64/64 pass**. The remaining ~3 500 LOC of structure-factor / nematic / VBS / plaquette / HDF5-results-writer / scan-mode logic will be peeled off in follow-up commits (one logical group per commit) so `ctest` stays green between commits. The GPU twin still carries its own drifted `Cluster` copy and will be re-pointed at `ed::bfg::Cluster` in a separate commit once the API surface there is finalised.
...
- [x] **P2.14 — DSSF PR-H**: delete deprecated `--dssf` flag in `ed_main.cpp`, delete `src/apps/TPQ_DSSF.cpp`, drop the never-implemented `--dssf-legacy-output` writer. *Done (current rollout):* the user requested the "newest version with one clean cpp caller for all the ED core routines (including dssf computation)", so the deferred backwards-compatibility window was closed early. Concretely: (1) `src/apps/TPQ_DSSF.cpp` (4 174 LOC monolith with its own 14-positional CLI, parallel-HDF5 plumbing, and a duplicated `/dssf_results/...` schema) was deleted outright; (2) `cmake/EDLibraries.cmake` + `CMakeLists.txt` lost the `TPQ_DSSF` `add_executable` target, the `HDF5 PARALLEL` plumbing it pulled in, and the `install(TARGETS TPQ_DSSF ...)` rule; (3) `.github/workflows/ci.yml` no longer compiles `TPQ_DSSF` in the CUDA build-only lane; (4) `src/apps/ed_main.cpp` lost its 394-line `run_dssf_mode` shim and the `--dssf` argv path, replaced by an explicit `--dssf` rejection that points users at `ED dssf <method>`; (5) `print_help` rewrote the "DSSF MODE (TPQ_DSSF-style interface)" block as the canonical "DSSF / SSSF SUBCOMMAND" section; (6) ten unused includes were pruned from `ed_main.cpp`; (7) `include/ed/dssf/dssf_io.h` docstring now records `TPQ_DSSF.cpp (deleted in P2.14)` as historical context; (8) `README.md` swapped the "DSSF Mode (Simplified Spectral Interface)" + "TPQ_DSSF Executable" sections for one "DSSF / SSSF Subcommand" section + a migration note, and renumbered the TOC; (9) `configs/13_tpq_dssf_workflow.cfg` was deleted and `configs/15_ed_dssf_mode.cfg` rewritten as the canonical `ED dssf <method>` worked example; (10) the `--dssf-legacy-output` writer was confirmed never to have been implemented (it only ever existed in the audit doc), so no code drop was needed beyond updating `dssf_io.h`. Net change: **−4 174 LOC** (TPQ_DSSF.cpp deletion) **−394 LOC** (run_dssf_mode in ed_main.cpp) plus a few hundred LOC of CMake/CI/help-text cleanup. ctest: 102/102 pass; pytest: 81/81 pass. With this slice `ED dssf <method>` is the *only* supported DSSF entry point, fulfilling the §3.10 promise of one canonical CLI for every ED core routine. (Earlier deferral note preserved below for archaeology.)
...
| `qed` Python rename breaks `import edlib` in user notebooks | Low | Medium | `python/edlib/__init__.py` becomes a thin `from qed import *  # DeprecationWarning` shim; both names install side-by-side for one release. |
...
3. **Python package name**: **`qed`** (PyPI-available, unambiguous; no PyPI conflict like `edlib`).

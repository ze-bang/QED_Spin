...
// P2.1: Cluster + load_cluster moved into the ed_bfg static library so
// `compute_bfg_order_parameters_gpu.cu`, future Python bindings, and the
// in-progress library split below the main() driver can all share the same
// authoritative implementation. Pull them into the local TU's namespace so
// the surrounding code keeps using the unqualified name.

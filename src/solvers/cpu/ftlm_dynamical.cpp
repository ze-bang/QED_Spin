...
#include <ed/solvers/ltlm.h>     // LTLMParameters (connected-QH LTLM estimator)
...
// Log-spaced temperature grid used by the LTLM connected-QH estimator
// (moved with it from ltlm.cpp in WP10 C7).
...
/**
 * @brief Compute ∂T⟨O⟩ = (⟨OH⟩ - ⟨O⟩⟨H⟩) / T² from the lowest LTLM Ritz states.
 *
 * WP10 C7: moved verbatim from ltlm.cpp (retired) so both connected-response
 * estimators -- the FTLM trace above and the LTLM low-lying projection here --
 * live next to each other. Seeding and numerics are unchanged.
 */

...
# ---------------------------------------------------------------------------
def gs_configs(n, gen):
    h = n // 2
    return [
        ("none",                dict(symmetry="off", auto_sz=False)),
        ("U(1)",                dict(symmetry="off", sz=h)),
        ("spatial",             dict(symmetry=gen, auto_sz=False)),
        ("U(1)+spatial",        dict(symmetry=gen, sz=h,
        ...
        ("U(1)+spatial+flip+TR", dict(symmetry=gen, sz=h,
                                      spin_flip="require",
                                      time_reversal="require")),
    ]


def thermal_configs(gen):
    return [
        ("none",                dict(symmetry=None,
                                     use_sz_if_conserved=False)),
        ("U(1)",                dict(symmetry=None)),
        ("U(1)+spatial",        dict(symmetry=gen,
                                     spin_flip="off", time_reversal="off")),
        ("U(1)+spatial+flip",   dict(symmetry=gen, spin_flip="require",
                                     time_reversal="off")),
        ("U(1)+spatial+TR",     dict(symmetry=gen, spin_flip="off",
                                     time_reversal="require")),
        ("U(1)+spatial+flip+TR", dict(symmetry=gen, spin_flip="require",
                                      time_reversal="require")),
    ]


def run_gs(H, n, gen, device, truth, rows):
    for label, kw in gs_configs(n, gen):
...
def run_thermal(H, n, gen, device, truth, rows):
    t_lo, t_hi, n_t = T_GRID
    for label, kw in thermal_configs(gen):
...
def main():
    ...
    rows: list[dict] = []
    for dev in devices:
        run_gs(H, n, gen, dev, truth, rows)
        run_thermal(H, n, gen, dev, truth, rows)

...
- [~] **P2.1** Refactor `compute_bfg_order_parameters.cpp` into `src/bfg/` library + ~50-line `main`. *In progress -- fourth slice landed:* on top of the first three slices (`Cluster` + `load_cluster`, topology + correlations, wavefunction I/O), this slice promotes the bond-bilinear structure-factor and Fourier-applied dimer kernels into `ed_bfg`: `include/ed/bfg/structure_factor.h` + `src/bfg/structure_factor.cpp` ship `DimerSFResult`, `compute_dimer_sf_direct`, `compute_heisenberg_sf_direct`, `apply_dimer_fourier`, `apply_heisenberg_dimer_fourier`, `compute_dimer_dimer_correlation`, `compute_heisenberg_dimer_dimer_correlation`, plus the `set_memory_efficient_mode` / `memory_efficient_mode_enabled` accessors that wrap the previously file-local global. The CPU driver (`compute_bfg_order_parameters.cpp`) loses ~660 LOC of duplicated kernels (3584 -> 2929 LOC) and now `using`-aliases every moved symbol; remaining call sites were swept from the old global to the accessor function. Catch2 lockdown: `tests/unit/test_bfg_structure_factor.cpp` (7 cases) covers the four kernels, the dimer-dimer correlations, and the memory-efficient flag. Pybind11 bindings expose the same surface under `quantum_ed.bfg` (`compute_dimer_sf_direct`, `compute_heisenberg_sf_direct`, `apply_dimer_fourier`, `apply_heisenberg_dimer_fourier`, the two `compute_*_dimer_dimer_correlation` overloads, `set_memory_efficient_mode`, `memory_efficient_mode_enabled`, and the `DimerSFResult` class). 7 new pytest cases (`python/tests/test_bfg.py`) round-trip the analytic 2-site states from the Catch2 suite through the bindings. ctest: **89/89 pass** (was 82/82); pytest: **71/71 pass** (was 64/64). The remaining ~2 900 LOC is now bowtie/triangle/VBS/plaquette/HDF5-results-writer/scan-mode logic; future commits will peel them off in similarly-scoped slices. *(Earlier slice notes preserved below for archaeology.)*

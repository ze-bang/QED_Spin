...
- [~] **P2.1** Refactor `compute_bfg_order_parameters.cpp` into `src/bfg/` library + ~50-line `main`. *In progress -- seventh slice landed:* on top of the previous six slices (`Cluster` + `load_cluster`, topology + correlations, wavefunction I/O, dimer structure factors, ring observables, spin structure factor), this slice promotes the HDF5 results writers and their scalar-summary record types into `ed_bfg::results_io`: `include/ed/bfg/results_io.h` + `src/bfg/results_io.cpp` ship `save_results`, `save_temperature_scan_results`, `save_scan_results`, plus the canonical struct definitions (`NematicResult`, `VBSResult`, `PlaquetteResult`, `Sq2DGridResult`, `OrderParameterResults`). The on-disk schema is preserved bit-for-bit (HDF5 dataset names, attribute names, the orientation index map `0=(0,0), 1=(0,1), 2=(0,2), 3=(1,1), 4=(1,2), 5=(2,2)`, and the spsm/szsz/heisenberg bond compound types). The CPU driver loses ~816 LOC (2645 -> 1829 LOC) and now `using`-aliases every moved type and writer. The remaining file is now nematic / VBS / plaquette / sq-2d-grid kernels + the `compute_all_order_parameters` orchestration + scan-mode glue + the `int main()` argv driver; future slices will peel those off as well. Pybind11 bindings for these record/writer types are intentionally deferred -- the result aggregates are only used internally by the driver, not by user-facing notebooks (the kernels feeding them are already exposed via `compute_dimer_sf_direct`, `compute_spin_structure_factor`, etc.). ctest: **102/102 pass** (unchanged); pytest: **81/81 pass** (unchanged). *(Earlier slice notes preserved below for archaeology.)*

...
- [~] **P2.1** Refactor `compute_bfg_order_parameters.cpp` into `src/bfg/` library + ~50-line `main`. *In progress -- eighth slice landed:* on top of the previous seven slices, this slice promotes the order-parameter physics kernels themselves into `ed_bfg::order_parameters`: `include/ed/bfg/order_parameters.h` + `src/bfg/order_parameters.cpp` ship `compute_nematic_order(_real)`, `compute_vbs_order`, `compute_plaquette_order`, `compute_sq_2d_grid`, and `compute_all_order_parameters`. With every kernel and POD now in the library, the CPU driver collapses to a thin orchestration shell: `process_all_temperatures` (TPQ-temperature sweep), `scan_jpm_directories` (per-Jpm parallel/MPI distribution), `print_usage`, and `int main()`. The driver also drops the dead bit / spin-operator helpers (`get_bit`, `set_bit`, `flip_bit`, `apply_sp`, `apply_sm`, `sz_value`) that lingered after earlier kernel extractions, the dead `PI`/`I` constants, and the unreferenced subset of using-declarations / std-headers. Net change: CPU driver **2645 -> 816 LOC** (-1 829 LOC across slices 7-8, of which -1 013 LOC came from this slice alone). All numerical and console-progress behaviour is preserved verbatim (the implementation moved, not the logic). ctest: **102/102 pass** (unchanged); pytest: **81/81 pass** (unchanged). The remaining ninth slice splits the orchestration shell itself (`process_all_temperatures` + `scan_jpm_directories` + `print_usage`) into a small `ed_cli` BFG sub-module so `main()` shrinks to the canonical ~50-line argv driver promised by the audit. *(Earlier slice notes preserved below for archaeology.)*

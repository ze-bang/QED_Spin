...
- [ ] **P0.1** Add `.gitignore` (covers `build/`, `*.h5`, scratch dirs, `__pycache__/`).
- [ ] **P0.2** Add `.editorconfig` (matches `.clang-format`).
- [ ] **P0.3** Add `.clang-format` (LLVM base, 4-space indent, 100-col, no `clang-format -i` yet).
- [ ] **P0.4** Add `CMakePresets.json` (`default`, `debug-asan`, `release-cuda`).
- [ ] **P0.5** Add minimal `.github/workflows/ci.yml` — single Linux/GCC-13/OpenBLAS lane, `cmake --preset default && cmake --build build && ctest --test-dir build`. Get the green badge on `README.md`.
- [ ] **P0.6** Add `CONTRIBUTING.md`, `CHANGELOG.md`, `CITATION.cff`.
- [ ] **P0.7** Add `.clang-tidy` (read-only — warnings, not errors, in CI).
- [ ] **P0.8** Add `.pre-commit-config.yaml` (clang-format, end-of-file-fixer, trailing-whitespace, no-commit-to-master).
- [ ] **P0.9** **Delete DMRG entirely**: `include/ed/dmrg/`, `src/ed/dmrg/` (if any), `test_dmrg_vs_ed` CMake target, README "DMRG (experimental)" section, any `--dmrg` flags.
- [ ] **P0.10** Remove the two hardcoded `/home/pc_linux/...` paths in `CMakeLists.txt` (lines 512–513, 531) — gate behind `ED_LAPACKE_ROOT` / `ED_BLAS_SHIM_DIR` cache variables (default empty).
- [ ] **P0.11** Run `clang-format -i` once across the codebase (one big formatting-only commit, no logic changes).
- [ ] **P0.12** Replace all `system("mkdir -p ...")` with `std::filesystem::create_directories` (~12 files, ~30 lines).
- [ ] **P0.13** Convert all `#ifndef X_H / #define X_H / #endif` to `#pragma once` (one sed pass).
- [ ] **P0.14** Move duplicated `enum class DiagonalizationMethod`, `EDParameters`, `EDResults` into `include/ed/core/ed_types.h`; both `ed_wrapper.h` and `src/core/ed_config.cpp` include it. Delete the "MUST stay in sync" comment.
- [ ] **P0.15** Drop bespoke JSON parser in `construct_ham.h`; add `nlohmann/json` via `FetchContent`. Replace `loadMaxClique`, `loadMinimalGenerators`, `loadSectorMetadata` with `nlohmann::json::parse(...)`.
- [ ] **P0.16** Drop `-ffast-math` from default flags; gate behind `ED_ENABLE_FAST_MATH` (default OFF). Run `bench_full*` before/after to confirm no regression.
...
- [ ] **P1.1** Split `CMakeLists.txt` into `cmake/EDOptions.cmake`, `cmake/EDCompilerFlags.cmake`, `cmake/EDBlasBackend.cmake`, `cmake/EDScaLAPACK.cmake`, `cmake/EDInstall.cmake`, `cmake/modules/FindARPACK.cmake`, `cmake/modules/FindScaLAPACK.cmake`. Top-level `CMakeLists.txt` becomes ~80 lines.
- [ ] **P1.2** Introduce `add_library(ed_core ...)`, `ed_solvers_cpu`, `ed_solvers_gpu`, `ed_io` static libraries. Apps link them.
- [ ] **P1.3** Header → source split for `ed_wrapper.h` (one PR).
- [ ] **P1.4** Header → source split for `construct_ham.h` (one PR).
- [ ] **P1.5** Header → source split for `hdf5_io.h` (one PR).
...
- [ ] **P1.13** Wire `clang-tidy` in CI (warnings only, not failures).
